...
public class CustomDialog extends Dialog {
	...
	/**
	 * Set dialog title text gravity. Note:If the icon has been set, title text
	 * always Gravity.LEFT.
	 * 
	 * @param gravity
	 * @return
	...
	public CustomDialog setTitleTextGravity(int gravity) {
		mTvTitle.setGravity(mIvIcon.getVisibility() == View.VISIBLE ? Gravity.LEFT
				: gravity);
		return this;
	}
		...
		mBtnClose.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				if (mColseClickListener != null) {
					mColseClickListener
							.onClick(CustomDialog.this, BUTTON_CLOSE);
				}
			}
		});
		return this;
	 ...
	 * 
	 * @return The text in input EditText, but null text when input EditText is
	 *         not visibility.
	 */
	...
	/**
	 * Set dialog input text. If input EditText dose not visibility, it would't
	 * set.
	 * 
	 * @param text
	 */
		...
		mBtnPositive.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				if (mPositiveClickListener != null) {
					mPositiveClickListener.onClick(CustomDialog.this,
		...
		mBtnNegative.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				if (mNegativeClickListener != null) {
					mNegativeClickListener.onClick(CustomDialog.this,
	...
	private void layoutButtons() {
		...
		if (mBtnPositive.getVisibility() == View.VISIBLE
				&& mBtnNegative.getVisibility() != View.VISIBLE) {
		...
		} else if (mBtnPositive.getVisibility() != View.VISIBLE
				&& mBtnNegative.getVisibility() == View.VISIBLE) {
			LinearLayout.LayoutParams lp = (LinearLayout.LayoutParams) mBtnNegative
					.getLayoutParams();
			lp.leftMargin = windowWidth / 4;
			lp.rightMargin = windowWidth / 4;
			mBtnNegative.setLayoutParams(lp);
				...
				&& mBtnNegative.getVisibility() == View.VISIBLE) {
			LinearLayout.LayoutParams PositiveLp = (LinearLayout.LayoutParams) mBtnPositive
					.getLayoutParams();
			PositiveLp.leftMargin = 0;
			PositiveLp.rightMargin = getContext().getResources()
					.getDimensionPixelSize(R.dimen.custom_dialog_button_margin);
			mBtnPositive.setLayoutParams(PositiveLp);
			LinearLayout.LayoutParams negativeLp = (LinearLayout.LayoutParams) mBtnNegative
					.getLayoutParams();
			negativeLp.leftMargin = getContext().getResources()
					.getDimensionPixelSize(R.dimen.custom_dialog_button_margin);
	 ...
	 *            Icons array.
	 * @param textGravity
	 *            Item text align. Only effect when icons is null, others it
	 *            would be Gravity.LEFT.
	 * @param listener
	 * @return
	 */
	public CustomDialog setItems(CharSequence[] items, int[] icons,
			int textGravity, DialogInterface.OnClickListener listener) {
		this.mItemClickListener = listener;
		if (items != null && items.length > 0) {
			if (icons != null && icons.length != items.length) {
				throw new IllegalArgumentException(
						"The items length dosen't equals to icons length.");
			}
			mFlBody.setVisibility(View.VISIBLE);
			mTvMessage.setVisibility(View.GONE);
			mListView.setVisibility(View.VISIBLE);
			mListView.setChoiceMode(AbsListView.CHOICE_MODE_NONE);
			ItemsAdapter adapter = new ItemsAdapter(getContext(), icons, items,
					textGravity);
					...
					.setOnItemClickListener(new AdapterView.OnItemClickListener() {
						...
						public void onItemClick(AdapterView<?> parent,
							...
							if (mItemClickListener != null) {
								mItemClickListener.onClick(CustomDialog.this,
										position);
							}
						}
					});
	 ...
	 * @return
	 */
	public CustomDialog setItems(CharSequence[] items, int textGravity,
			DialogInterface.OnClickListener listener) {
	...
	/**
	 * Returns the currently checked item. Note: The result is only valid if the
	 * items is
	 * {@link CustomDialog#setSingleChoiceItems(CharSequence[], int, android.content.DialogInterface.OnClickListener)}
	 * (include all of method named setSingleChoiceItems())
	 * 
	...
	/**
	 * Returns the currently checked items. Note: The result is only valid if
	 * the items is
	 * {@link CustomDialog#setMultiChoiceItems(CharSequence[], int[], android.content.DialogInterface.OnMultiChoiceClickListener)}
	 * (include all of method named setMultiChoiceItems())
	 * 
	 * @return The position of the currently checked item or null if items are
	 *         not multiple choice.
	 */
	...
	private class ItemsAdapter extends BaseAdapter {
		...
		public ItemsAdapter(Context context, int[] icons, CharSequence[] items,
				int gravity) {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if (convertView == null) {
				...
				convertView = View.inflate(mmContext, R.layout.item_dialog,
						null);
				viewHolder.icon = (ImageView) convertView
						.findViewById(R.id.iv_item_dialog_icon);
				viewHolder.text = (TextView) convertView
						.findViewById(R.id.tv_item_dialog_text);
				viewHolder.icon.setVisibility(mmIcons == null ? View.GONE
						: View.VISIBLE);
